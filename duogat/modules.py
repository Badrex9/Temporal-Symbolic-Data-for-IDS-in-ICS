...
class TemporalAttentionLayer(nn.Module):
    ...
    def forward(self, x, dif_x):
        ...
        # Do NOT recreate them with torch.tensor(...), otherwise they go back to CPU.
        if not torch.is_tensor(x):
            x = torch.as_tensor(x, dtype=torch.float32, device=self.a.device)
        else:
            x = x.to(device=self.a.device, dtype=torch.float32)

        if not torch.is_tensor(dif_x):
            dif_x = torch.as_tensor(dif_x, dtype=torch.float32, device=self.a.device)
        else:
            dif_x = dif_x.to(device=self.a.device, dtype=torch.float32)
